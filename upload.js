...
async function uploadBatch(state,onRefresh){
  ...
  for(let i=0;i<files.length;i++){
    ...
    const p=await supabase.from('fid_photos').insert({species_id:speciesId,storage_path:path,original_filename:file.name,orientation_degrees:orientation,is_active:true}).select('id').single();
