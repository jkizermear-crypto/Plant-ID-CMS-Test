...
export async function deleteTip(id){return supabase.from('fid_tips').delete().eq('id',id)}
...
export async function deleteComparison(id){return supabase.from('fid_comparisons').delete().eq('id',id)}

export async function loadDistractors(){
  return supabase.from('fid_distractor_options')
    .select('id,species_id,distractor_species_id,distractor_label,sort_order,release_id,publish_status,species:fid_species!fid_distractor_options_species_id_fkey(common_name),distractor:fid_species!fid_distractor_options_distractor_species_id_fkey(common_name)')
    .order('sort_order');
}
...
export async function loadReleases(){
  return supabase.from('fid_releases').select('*').order('version',{ascending:false});
}
export async function saveReleaseBanner(id,payload){
  return supabase.from('fid_releases').update(payload).eq('id',id);
}
export async function saveReleaseSettings(id,payload){
  return supabase.from('fid_releases').update(payload).eq('id',id).select('*').single();
}
export async function loadReleaseCounts(){
  return supabase.from('fid_release_content_counts').select('*').order('version',{ascending:false});
}
