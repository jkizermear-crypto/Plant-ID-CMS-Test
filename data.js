...
export async function replaceDistractors(speciesId,distractorIds,labelOnly=[],releaseMeta=null){
  const d=await supabase.from('fid_distractor_options').delete().eq('species_id',speciesId);
  if(d.error)return d;
  const rows=[
    ...distractorIds.map((id,i)=>({species_id:speciesId,distractor_species_id:id,distractor_label:null,sort_order:i,...(releaseMeta||{})})),
    ...labelOnly.filter(Boolean).map((label,i)=>({species_id:speciesId,distractor_species_id:null,distractor_label:label,sort_order:distractorIds.length+i,...(releaseMeta||{})}))
  ];
...
export async function saveRelease(id,payload){
  return supabase.from('fid_releases').update(payload).eq('id',id);
}
export async function loadReleaseCounts(){
  return supabase.from('fid_release_content_counts').select('*').order('version');
}

export async function createRelease(version,title){
  return supabase.rpc('fid_create_release',{
    p_version:version,
    p_title:title
  });
}
...
export async function publishRelease(releaseId){
  return supabase.rpc('fid_publish_release',{
