...
import {renderBatch71Page} from './batch71.js';
...
function renderShell(){
  ...
  <main class="wrap"><div id="workingReleaseBar" class="card" style="margin-top:12px"></div><div class="tabs"><button class="tab active" data-view="dashboard">Dashboard</button><button class="tab" data-view="releases">Releases</button><button class="tab" data-view="species">Species</button><button class="tab" data-view="photos">Photos</button><button class="tab" data-view="upload">Bulk Upload</button><button class="tab" data-view="batch71">7.1 Batch Import</button><button class="tab" data-view="tips">ID Cards</button><button class="tab" data-view="comparisons">Comparisons</button><button class="tab" data-view="distractors">Distractors</button><button class="tab" data-view="preview">Preview</button><button class="tab" data-view="release">Release Banner</button></div><div id="page"></div></main></div>`;
...
function setView(v){state.view=v;$$('.tab').forEach(b=>b.classList.toggle('active',b.dataset.view===v));if(v==='dashboard')renderDashboard();if(v==='releases')renderReleasesPage(state,refreshAll);if(v==='species')renderSpeciesPage(state,refreshAll);if(v==='photos')renderPhotosPage(state,refreshAll);if(v==='upload')renderUploadPage(state,refreshAll);if(v==='batch71')renderBatch71Page(state,refreshAll);if(v==='tips')renderTipsPage(state,refreshAll);if(v==='comparisons')renderComparisonsPage(state,refreshAll);if(v==='distractors')renderDistractorsPage(state,refreshAll);if(v==='preview')renderPreviewPage(state);if(v==='release')renderReleaseBannerPage(state,refreshAll);}
